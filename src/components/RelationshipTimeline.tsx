...
export function RelationshipTimeline({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="연도별 관계 흐름">
      ...
      <polyline points={pts} fill="none" stroke="#7C6CD8" strokeWidth={2} />
      {/* 최고/최저 */}
      ...
      {timeline.filter((_, i) => i % 2 === 0).map((p) => (
        <text key={p.year} x={xOf(p.year)} y={H - padBot + 16} fontSize={9} fill="rgba(44,39,56,.5)" textAnchor="middle" style={{ fontFamily: "var(--font-display)" }}>{`'${String(p.year).slice(2)}`}</text>
      ))}
    </svg>
  );
}
